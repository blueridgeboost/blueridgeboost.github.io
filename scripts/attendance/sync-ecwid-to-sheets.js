...
async function main() {
    ...
    const requiredClassCols = ['brb_id','ecwid_product_id','class_name','teacher_emails','day_of_week','start_date','end_date','class_status'];
    for (const c of requiredClassCols) {
        if (classIdx(c) === -1) throw new Error(`Missing required column ${c} in Classes tab`);
    }
    ...
    for (const c of requiredEnrollCols) {
        if (enrollIdx(c) === -1) throw new Error(`Missing required column ${c} in Enrollments tab`);
    }
    ...
    for (const c of classes) {
        ...
        if (!brbId) {
            console.warn(`Class ${c.name} is missing brb_id, skipping`);
            continue;
        }

        // retrieve items from ecwid
        const classType = getAttributeValue(c, 'class_type');
        const teacherEmails = getAttributeValue(c, 'teacher_emails'); // suggest adding teacher_emails to ecwid products
        ...
        // --- write classes row according to sheet headers --- 
        ...
        for (const order of orders) {
            ...
            for (const item of (order.items || [])) {
                ...
                const getExtra = (title) => order?.orderExtraFields?.find(f => f?.title === title)?.value;
                ...
                // if quantity > 1, add second student
