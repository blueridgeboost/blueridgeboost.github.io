...
async function sendEmail({ parentEmail, parentName, discountCode }) {
  ...
  // response is an array of { email, status, _id, reject_reason }
  const result = response?.[0];
  if (!result) throw new Error(`No response from Mailchimp for ${parentEmail}`);

  if (result.status !== 'sent' && result.status !== 'queued') {
    throw new Error(
      `Mailchimp rejected ${parentEmail}: status=${result.status}, reason=${result.reject_reason ?? 'unknown'}`
    );
  }
